...
#Note the difference in columns between the two data frames
#differences between data frames: pre_2007 has 'CO2' 2007-2021 has 'Depth_m'


```

## Format pre 2007 data. First determine how to fix the date
...
#testing combining both changes into one line
ccr_dates_test <- as.Date(str_extract(ccr_2007_2021$Date, "[0-9]+/[0-9]+/[0-9]+"), "%m/%d/%Y")
  view(ccr_dates_test)

#### changing other variables to numeric
#Old formatting
# #Pre 2007
# ccr_pre_2007_filtered <- ccr_pre_2007 %>%
...
# ccr_pre_2007_filtered <- ccr_pre_2007 %>%
#   mutate(Algae_cells_mL = as.numeric(Algae_cells_mL),
#          Fe_mgL = as.numeric(Fe_mgL))
# ccr_pre_2007_filtered <- ccr_pre_2007 %>%
#   mutate(Mn_mgL = as.numeric(Mn_mgL),
#          Alkalinity = as.numeric(Alkalinity))
# ccr_pre_2007_filtered <- ccr_pre_2007 %>%
#   mutate(Hardness = as.numeric(Hardness),
#          CO2 = as.numeric(CO2))
# ccr_pre_2007_filtered <- ccr_pre_2007 %>%
#   mutate(Reservoir_level = as.numeric(Reservoir_level),
#          Elevation = as.numeric(Elevation))
# 
# #2007-2021
# 
# ccr_2007_2021_filtered <- ccr_pre_2007 %>%
#   mutate(Temp_C = as.numeric(Temp_C),
#          pH = as.numeric(pH))
# ccr_2007_2021_filtered <- ccr_pre_2007 %>%
#   mutate(DO_mgL = as.numeric(DO_mgL),
...
## Format the pre 2007 data 

```{r}
## Cleaning up pre 2007 data
#first lets look at the classes of variables again
head(ccr_pre_2007)

#Now we're going to reformat the date and change non numeric values to numerics
ccr_pre_2007_filtered <- ccr_pre_2007 %>%
  mutate(Date = as.Date(str_extract(Date, "[0-9]+/[0-9]+/[0-9]+"), "%m/%d/%y")) %>% 
  mutate(Temp_C = as.numeric(Temp_C),
         pH = as.numeric(pH),
         ...
         Odor = as.numeric(Odor),
         Algae_cells_mL = as.numeric(Algae_cells_mL),
         Fe_mgL = as.numeric(Fe_mgL),
         Mn_mgL = as.numeric(Mn_mgL),
         Alkalinity = as.numeric(Alkalinity),
         Hardness = as.numeric(Hardness),
         CO2 = as.numeric(CO2),
         Reservoir_level = as.numeric(Reservoir_level))
         
  



class(ccr_pre_2007_filtered$Temp_C)

```


## Format the 2007-2021 data
...
#Now we're going to reformat the date and change non numeric values to numeric
ccr_2007_2021_filtered <- ccr_2007_2021 %>%
  mutate(Date = as.Date(str_extract(Date, "[0-9]+/[0-9]+/[0-9]+"), "%m/%d/%Y")) %>% #NOTE the capital Y here since years are written as 2007 instead of 07
  mutate(Temp_C = as.numeric(Temp_C),
         pH = as.numeric(pH),
         DO_mgL = as.numeric(DO_mgL),
         Turb = as.numeric(Turb),
...
rbind(ccr_pre_2007_filtered, ccr_2007_2021_filtered) #but this doesn't capture the unique columns in the two dataframes

#adding missing columns for each df
ccr_2007_2021_filtered$CO2 <- NA
ccr_pre_2007_filtered$Depth_m <- NA

binded_data <- rbind(ccr_pre_2007_filtered, ccr_2007_2021_filtered)

#arange by descending data 
...
## Write csv of binded data
```{r}

write.csv(binded_data, "./Final_CCR_WVWA_data_binded.csv", row.names = FALSE)
