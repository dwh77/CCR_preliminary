...
ccr <- ctda %>% 
  ...
  select(Reservoir, Site, Date, Depth_m, Temp_C, DO_mgL) %>% 
...
##Plots for 2013 and 2019, need to run each year through df.final command to make easier plotting
{
  ...
  mytheme <- theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
                   ...
                   #legend.position = c(22, 0.5),
                   axis.text=element_text(size=20),
                   axis.title=element_text(size=20,face="bold"),
                   plot.title = element_text(size = 20, face = "bold", hjust = 0.5))
  
  DO13 <- ggplot(data = ctd, mapping = aes(y = Depth_m, x = DO_mgL))+
    geom_point(color = "blue", size = 1)+
    geom_line(orientation = "y",color = "blue")+   # so points are connected based on order of depth than of DO 
    labs(x = "DO (mg/L)", y = "Depth (m)")+
    scale_y_continuous(trans = "reverse")+
    #ggtitle("CCR CTD cast 2013-08-22")+
    mytheme
  DO13
  
  WTR13 <- ggplot(data = ctd, mapping = aes(x = Temp_C, y = Depth_m))+
  ...
  drivers <- ggarrange(DO13, WTR13, 
                       ncol = 2, nrow = 1) 
  drivers
  
  # ggsave(filename = "./CCR_plots/CCR_CTD_22August2013.tiff", 
  #        drivers, device = "tiff", width = 230, height = 140, units = "mm")
  # 
  
  DO19 <- ggplot(data = ctd, mapping = aes(y = Depth_m, x = DO_mgL))+
    geom_point(color = "blue", size = 1)+
    geom_line(orientation = "y",color = "blue")+   # so points are connected based on order of depth than of DO 
    labs(x = "DO (mg/L)", y = "Depth (m)")+
    scale_y_continuous(trans = "reverse")+
  ...
  WTR19 <- ggplot(data = ctd, mapping = aes(x = Temp_C, y = Depth_m))+
    geom_point(color = "black", size = 1)+
    geom_line(color = "black", size = 1)+
    labs(x = "Temp (C)", y = "Depth (m)")+
    scale_y_continuous(trans = "reverse")+ 
  ...
  drivers <- ggarrange(DO19, WTR19, 
                       ncol = 2, nrow = 1) 
  drivers
  
  # ggsave(filename = "./CCR_plots/CCR_CTD_28June2019.tiff", 
  #        drivers, device = "tiff", width = 230, height = 140, units = "mm")
...
#dissolved oxygen
interp_do <- interp(x=do$DOY, y = do$Depth_m, z = do$DO_mgL,        #ditto to lines for temp 
