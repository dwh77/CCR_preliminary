...
ccr <- ctda %>% 
  ...
  select(Reservoir, Site, Date, Depth_m, Temp_C, DO_mgL, Flag_Temp, Flag_DO) %>% 
...
p1 <- ggplot(interp_temp, aes(x= as.Date(x, origin = "1970-01-01"), y=y))+
 ...
 # geom_line(data = FCR_thermo_18, aes(x=Date, y=thermo.depth, z=NULL), color = "black", lwd = 1)+
  ...
  labs(x = "Date", y = "Depth (m)", title = "CCR Temperature Heatmap",fill=expression(''*~degree*C*''))+ #x was day of year
  theme_black()
...
p5 <- ggplot(interp_do, aes(x= as.Date(x, origin = "1970-01-01"), y=y))+
  ...
  labs(x = "Date", y = "Depth (m)", title = "CCR Dissolved Oxygen Heatmap", fill="mg/L")+ #x was day of year 
  theme_black()
